...
class Ccc_VendorInventory_Model_Observer
{
    ...
    public function readcsv()
    {
        ...
        $config = Mage::getModel('vendorinventory/vendorinventory')->load($id, 'brand_id');
        var_dump($config->getId());
        $branddata = Mage::getModel('vendorinventory/configdata')->load($config->getId(),'config_id')->getData();
            print_r($branddata);
            $serial = unserialize($branddata['brand_data']);
            print_r($serial);
